...
static void chassis_feedback_update(chassis_move_t *chassis_move_update)
{
  ...
  chassis_move_update->classis_ramp.max_value = scopperil; 
}
/**
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
  ...
  else if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_TOP)
  {
    ...
    sin_yaw = arm_sin_f32(-chassis_move_control->chassis_yaw_motor->relative_angle-chassis_move_control->classis_ramp.out);
    cos_yaw = arm_cos_f32(-chassis_move_control->chassis_yaw_motor->relative_angle-chassis_move_control->classis_ramp.out);
...
//		fix_GravityCenter_rotation(&vx_set, &vy_set, &wz_set);
	
	
		wheel_speed[0] = -vx_set + vy_set + (CHASSIS_WZ_SET_SCALE - 1.0f) * (MOTOR_DISTANCE_TO_CENTER) * ROTATE_INERTIA_RATE_ONE * wz_set;
		wheel_speed[1] = -vx_set - vy_set + (CHASSIS_WZ_SET_SCALE - 1.0f) * (MOTOR_DISTANCE_TO_CENTER) * ROTATE_INERTIA_RATE_TWO * wz_set;
		wheel_speed[2] = +vx_set - vy_set + (-CHASSIS_WZ_SET_SCALE - 1.0f) * (MOTOR_DISTANCE_TO_CENTER) * ROTATE_INERTIA_RATE_THREE * wz_set;
		wheel_speed[3] = +vx_set + vy_set + (-CHASSIS_WZ_SET_SCALE - 1.0f) * (MOTOR_DISTANCE_TO_CENTER) * ROTATE_INERTIA_RATE_FOUR * wz_set;
}
...
chassis_move_t *GetChassisData(void)
{
  return &chassis_move;
}
...
void chassis_power_control(chassis_move_t *chassis_power_control)
{
  ...
  {
    get_chassis_power_and_buffer_OLD(&chassis_power, &chassis_power_buffer);
...
static void chassis_jude_mode(chassis_move_t *chassis_move_update)
{
  ...
  else if (Chassis_Power_Limit() == 60)
  {
    ...
    if (chassis_move_update->chassis_RC->key.v & CLASSIS_CHONGNEN_MODE)
    {
      ...
      chassis_move_update->vx_max_speed = NORMAL_MAX_CHASSIS_SPEED_X_60;
      chassis_move_update->vx_min_speed = -NORMAL_MAX_CHASSIS_SPEED_X_60;

      chassis_move_update->vy_max_speed = NORMAL_MAX_CHASSIS_SPEED_Y_60;
      chassis_move_update->vy_min_speed = -NORMAL_MAX_CHASSIS_SPEED_Y_60;
