/**
  ...
  *                    ?ò???????ò??/''\ ?????×????×?
  *
  *             ??????flash????°ü?¨??×?????????×? name[3] ?? ??×?±ê???? cali_flag
  *             ????head_cali??°???×???,???ü?è??12×?????flash,?????ü??0x080A0000????
  *             0x080A0000-0x080A0007: head_cali????
  *             0x080A0008: ??×?name[0]
  *             0x080A0009: ??×?name[1]
  *             0x080A000A: ??×?name[2]
  *             0x080A000B: ??×?±ê???? cali_flag,?±??×?±ê??????0x55,????×?head_cali??????×???
  *             ?í?????è±?
  *             1.?í???è±?????calibrate_task.h??cali_id_e, ??
  *             typedef enum
  *             {
  *                 ...
  *                 //add more...
  ...
  *                 uint16_t yyy;
  *                 fp32 zzz;
  *             } xxx_cali_t; //?¤??:8×??? 8 bytes, ±????? 4, 8, 12, 16...
  *             3.?? "FLASH_WRITE_BUF_LENGHT",?í??"sizeof(xxx_cali_t)", ????????????
  *             bool_t cali_xxx_hook(uint32_t *cali, bool_t cmd), ?í??????×??? "cali_name[CALI_LIST_LENGHT][3]"
  *             ???ê?÷±??? xxx_cali_t xxx_cail, ?í??±??????·??cali_sensor_buf[CALI_LIST_LENGHT]
  *             ??cali_sensor_size[CALI_LIST_LENGHT]?í???????¤??, ×??ó??cali_hook_fun[CALI_LIST_LENGHT]?í??????
  *
  ==============================================================================
  @endverbatim
  ****************************(C) COPYRIGHT 2019 DJI****************************
...
/**
  * @brief          "head"?è±???×?
  * @param[in][out] cali:???????òhead????,?±cmd??CALI_FUNC_CMD_INIT, ??????????,CALI_FUNC_CMD_ON,??????????
  * @param[in]      cmd: 
                    CALI_FUNC_CMD_INIT: ?ú±í????×???????????????????
                    CALI_FUNC_CMD_ON: ?ú±í?è????×?
  * @retval         0:??×????????????ê
                    1:??×??????????ê??
  */
...
/**
  * @brief          ???????è±???×?
  * @param[in][out] cali:???????ò??????????,?±cmd??CALI_FUNC_CMD_INIT, ??????????,CALI_FUNC_CMD_ON,??????????
  * @param[in]      cmd: 
                    CALI_FUNC_CMD_INIT: ?ú±í????×???????????????????
                    CALI_FUNC_CMD_ON: ?ú±í?è????×?
  * @retval         0:??×????????????ê
