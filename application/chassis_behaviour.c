...
void Provincial_Competition_mode(void);
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    {
			 if(chassis_move_mode->chassis_RC->rc.ch[4] > 500 || chassis_move_mode->chassis_RC->rc.ch[4] < -500)
					chassis_behaviour_mode = CHASSIS_TOP;
			 else			 
			  	Provincial_Competition_mode();
//        chassis_behaviour_mode = CHASSIS_TOP;
    }
...
//  rc_deadband_limit(chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_Y_CHANNEL], vy_channel, CHASSIS_RC_DEADLINE);
		...
		if(reset_yaw == 0)
		{
			vx_set_channel = 30 * CHASSIS_VX_RC_SEN;
			vy_set_channel = 50 * -CHASSIS_VY_RC_SEN;
		}
		else
		{
			vx_set_channel = 0 * CHASSIS_VX_RC_SEN;
			vy_set_channel = 0 * -CHASSIS_VY_RC_SEN;			
		}
